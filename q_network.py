    ...
    def __init__(self, state_size, action_size, model=None):
        if model is not None:
            ...
            self.model = Sequential([
                # Input layer
                Dense(64, activation='relu', input_shape=(state_size,), kernel_regularizer=l2(0.01)),
                Dense(64, activation='relu', kernel_regularizer=l2(0.01)),
                Dense(64, activation='relu', kernel_regularizer=l2(0.01)),
                Dense(action_size, activation='linear', kernel_regularizer=l2(0.01))
            ])
            self.model.compile(optimizer='nadam', loss=masked_huber_loss(0.0, 1.0), metrics=['accuracy'])
